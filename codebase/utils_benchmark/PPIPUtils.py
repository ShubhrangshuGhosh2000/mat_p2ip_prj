...
def calcScores_DS(labels, phats):
    ...
    # calculating precision and recall in D-Script specific way -start
    print('calculating precision and recall in D-Script specific way')
    guess_cutoff = np.array([0.5], dtype=np.float32)
    labels = labels.astype(np.float32)
    p_guess = (guess_cutoff * np.ones(len(labels)) < phats).astype(np.float32)
    correct = np.sum(p_guess == labels).item()
    tp = np.sum(labels * phats).item()
    precision = tp / np.sum(phats).item()
    recall = tp / np.sum(labels).item()
    print('precision: ' + str(precision) + ' : recall: ' + str(recall))
    # calculating precision and recall in D-Script specific way -end

    # calculating precision and recall in SK-learn way -start
    print('calculating precision and recall in SK-learn way')
    y_hat = np.round(phats).astype(int)
    precision = metrics.precision_score(labels, y_hat)
    recall = metrics.recall_score(labels,y_hat)
    print('precision: ' + str(precision) + ' : recall: ' + str(recall))
    # calculating precision and recall in SK-learn way -end
...
def getUniprotFastaLocations(getBoth=True):
    ...
    uniprotFastaLoc = os.path.join(path_root, 'utils_benchmark', 'uniprot_sprot.fasta')
    uniprotFastaLoc2 = os.path.join(path_root, 'utils_benchmark', 'uniprot_trembl.fasta')
