...
def loadAAData(aaIDs):
    ...
    aaIdx = open(os.path.join(path_root, 'utils_benchmark/AAidx.txt'))
    aaData = []
    for line in aaIdx:
...
def loadPairwiseAAData(aaIDs,AADict=None):
    ...
    for item in aaIDs:
        if item == 'Grantham':
            ...
            f = open(os.path.join(path_root, 'utils_benchmark/Grantham.txt'))
        elif item == 'Schneider-Wrede':
            # f = open(currentdir+'/Schneider-Wrede.txt')
            f = open(os.path.join(path_root, 'utils_benchmark/Schneider-Wrede.txt'))
...
def loadBlosum62():
    ...
    if blosumMatrix is None:
        ...
        f = open(os.path.join(path_root, 'utils_benchmark/blosum62.txt'))
        header = None
        blosumMatrix = []
        for line in f:
            line = line.strip()
