...
from proc.mtf_p2ip.mtf_p2ip_origMan_auxTlOtherMan.mtf_MtfP2ipNetwork_origMan_auxTlOtherMan_held20 import MtfP2ipNetworkModule
from utils.ProjectDataLoader import *
from utils.feat_engg_manual_main import extract_prot_seq_2D_manual_feat
from proc.mtf_p2ip.mtf_p2ip_origMan_auxTlOtherMan import mtf_RunTrainTest_origMan_auxTlOtherMan_held20
...
baseResultsFolderName = os.path.join(root_path, 'dataset/proc_data/benchmark_res/mtf_res/mtf_res_origMan_auxTlOtherMan_2d_htuned/')
...
def RunAll():    
    if chen2019RNN:
        ...
        #     # pipr_RunTrainTest_origMan_auxTlOtherMan_held50.runTest(MtfP2ipNetworkModule, outResultsName,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=convertToFolder(saves),predictionsFLst = pfs,startIdx=0)
        #     mtf_RunTrainTest_origMan_auxTlOtherMan_held50.runTest(MtfP2ipNetworkModule, outResultsName,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=None,predictionsFLst = pfs,startIdx=0)
        ...
        if HumanHeldOut20:
            ...
            # mtf_RunTrainTest_origMan_auxTlOtherMan_held20.runTestLst(MtfP2ipNetworkModule, outResultsNameLst,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=convertToFolder(saves),predictionsFLst = pfs,startIdx=0)
            mtf_RunTrainTest_origMan_auxTlOtherMan_held20.runTestLst(MtfP2ipNetworkModule, outResultsNameLst,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=None,predictionsFLst = pfs,startIdx=0)
