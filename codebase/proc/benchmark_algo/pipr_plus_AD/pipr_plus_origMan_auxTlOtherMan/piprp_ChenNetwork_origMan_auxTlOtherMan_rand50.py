...
from proc.benchmark_algo.pipr_plus_AD.GenericNetworkModel_piprp import GenericNetworkModel
from proc.benchmark_algo.pipr_plus_AD.GenericNetworkModule_piprp import GenericNetworkModule
from proc.benchmark_algo.pipr_plus_AD.positional_encoder_piprp import PositionalEncoder
import torch
...
class ChenNetworkModule(GenericNetworkModule):
    ...
    def loadFeatureData(self,featureFolder):
        ...
        print("\nloading AD_seq_feat_dict ...")
        # load tl-based 1d embeddings stored in AD_seq_feat_dict
        AD_seq_feat_dict_path = os.path.join(Path(__file__).parents[5], 'dataset/preproc_data_AD', 'AD_seq_1d_feat_dict_prot_t5_xl_uniref50.pkl')
        AD_seq_feat_dict = joblib.load(AD_seq_feat_dict_path)
        # trimming AD_seq_feat_dict so that it occupies less memory (RAM)
        for prot_id in list(AD_seq_feat_dict.keys()):
            AD_seq_feat_dict[prot_id]['seq'] = AD_seq_feat_dict[prot_id]['seq_2d_feat'] = None
        print("loaded AD_seq_feat_dict ...")

        print('\n#### loading other manual 1d embeddings ####')
        print("loading AD_seq_manual_feat_dict ...")
        AD_seq_manual_feat_dict = joblib.load(os.path.join(Path(__file__).parents[5], 'dataset/preproc_data_AD','AD_seq_manual_feat_dict.pkl'))
        # trimming AD_seq_manual_feat_dict so that it occupies less memory (RAM)
        for prot_id in list(AD_seq_manual_feat_dict.keys()):
            AD_seq_manual_feat_dict[prot_id]['seq'] = None
        print("loaded AD_seq_manual_feat_dict ...")
        ...
        allProteinsList = list(AD_seq_feat_dict.keys())
        ...
        # self.aux_oneDencodingsize = len(AD_seq_feat_dict[int(allProteinsList[0])]['seq_feat'])
        ...
        for item in allProteinsSet:
            ...
            tl_1d_embedd_tensor = torch.from_numpy(AD_seq_feat_dict[int(item)]['seq_feat'])
            # extract other manual 1d embeddings 
            seq_manual_feat_dict = AD_seq_manual_feat_dict[int(item)]['seq_manual_feat_dict']
