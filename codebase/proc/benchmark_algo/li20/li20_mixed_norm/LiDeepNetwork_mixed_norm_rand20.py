...
from proc.benchmark_algo.li20.LiModels import *
...
class LiDeepModel(object):
    def __init__(self,hyp={},fullGPU=False,deviceType=None,k=3,numEpochs=200,layer1Lr=2e-1,layer2Lr=1e-1,batchSize=256, minLr=1e-4, schedFactor=.1,schedPatience=2,schedThresh=5e-2):
        ...
        self.numEpochs = hyp.get('numEpochs',numEpochs)  # 3 ################################# FOR TEMP TESTING
        ...
        self.k = hyp.get('k',k)  # 2 ################################# FOR TEMP TESTING
        ...
        for i in range(0,k):
            self.models['AAC'][i] = [
            NetworkRunner(ModelLiDeep(1234,24,1,3,4,10,3,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1234,24,1,3,3,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1234,24,1,3,4,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1234,24,2,3,3,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp)
            ]
                        
            self.models['LD'][i] = [
            NetworkRunner(ModelLiDeep(1654,24,1,3,4,10,3,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1654,24,1,3,3,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1654,24,1,3,4,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1654,24,2,3,3,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp)
            ]
            
            self.models['CT'][i] = [
            NetworkRunner(ModelLiDeep(1367,24,1,3,4,10,3,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1367,24,1,3,3,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1367,24,1,3,4,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1367,24,2,3,3,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp)
            ]
            
            self.models['PAAC'][i] = [
            NetworkRunner(ModelLiDeep(1059,24,1,3,4,10,3,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1059,24,1,3,3,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1059,24,1,3,4,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp),
            NetworkRunner(ModelLiDeep(1059,24,2,3,3,10,2,self.seed),lr=self.layer1Lr,hyp=self.hyp)
            ]
    ...
    def fit(self,features,classes):
        layer2Features = []
        modelIdx=0
        netTypeLst = ['AAC','LD','CT','PAAC']
        folds = self.createKFoldIdx(features['AAC'].shape[0],classes,self.k,self.seed)
        # check for the intermediate results and if one exists, then resume from that checkpoint
        print('Checking for the intermediate results and if one exists, then resume from that checkpoint')
        temp_pkl_file_names_lst = glob.glob(os.path.join('temp_result', 'temp_mixed_norm_rand20_*.pkl'))
        print('Already existing result file names list: ' + str(temp_pkl_file_names_lst))
        if(len(temp_pkl_file_names_lst) > 0):
            # intermediate result exists; so load it in the layer2Features
            temp_res_file_name = temp_pkl_file_names_lst[0]
            layer2Features = joblib.load(temp_res_file_name)
            # adjust netTypeLst
            last_saved_netType = temp_res_file_name.replace('.pkl', '').split('_')[-1]
            print('last_saved_netType: ' + str(last_saved_netType))
            last_saved_netType_index = netTypeLst.index(last_saved_netType)
            netTypeLst = netTypeLst[last_saved_netType_index + 1 : ]
            print('modified netTypeLst: ' + str(netTypeLst))
            # adjust modelIdx
            last_saved_modelIdx = temp_res_file_name.replace('.pkl', '').split('_')[-2]
            print('last_saved_modelIdx: ' + str(last_saved_modelIdx))
            modelIdx = int(last_saved_modelIdx)

        for netType in netTypeLst:  #['AAC','LD','CT','PAAC']
            for netIdx in range(0,len(self.models[netType][0])):
                ...
                # end of for loop: for foldIdx in range(0,self.k):
                layer2Features.append(np.expand_dims(curValFeatures,1))
            # end of for loop: for netIdx in range(0,len(self.models[netType][0])):

            # save the intermediate result so that in case of abrupt termination, the flow can be started from the last saved result.
            # first, remove any already saved intermediate result
            print('\n Saving the intermediate result so that in case of abrupt termination, the flow can be started from the last saved result')
            print('First, remove any already saved intermediate result.')
            temp_pkl_file_names_lst = glob.glob(os.path.join('temp_result', 'temp_mixed_norm_rand20_*.pkl'))
            print('Already existing result file names list: : ' + str(temp_pkl_file_names_lst))
            for temp_pkl_file_name in temp_pkl_file_names_lst:
                os.remove(temp_pkl_file_name)
            # save the latest intermediate result
            print('Saving the latest intermediate result...')
            temp_pkl_file_nm_with_location = os.path.join('temp_result', 'temp_mixed_norm_rand20_' + str(modelIdx) + '_' + str(netType) + '.pkl')
            joblib.dump(value=layer2Features, filename=temp_pkl_file_nm_with_location, compress=3)
            print('The intermediate result is saved as: ' + temp_pkl_file_nm_with_location)

        # end of for loop: for netType in ['AAC','LD','CT','PAAC']:
...
class LiDeepNetworkModule(GenericModule):
    ...
    def loadFeatureData(self,featureFolder):
        self.featuresData = {}
        print('loading')
        self.featuresData['AAC'] = ProteinFeaturesHolder([featureFolder + 'AC30_mixed.tsv'])
        self.featuresData['PAAC'] = ProteinFeaturesHolder([featureFolder + 'PSAAC15_mixed.tsv'])
        self.featuresData['CT'] = ProteinFeaturesHolder([featureFolder + 'ConjointTriad_mixed.tsv'])
        self.featuresData['LD'] = ProteinFeaturesHolder([featureFolder + 'LD10_CTD_ConjointTriad_C.tsv',featureFolder + 'LD10_CTD_ConjointTriad_T.tsv',featureFolder + 'LD10_CTD_ConjointTriad_D_mixed.tsv'])
        print('done loading')
