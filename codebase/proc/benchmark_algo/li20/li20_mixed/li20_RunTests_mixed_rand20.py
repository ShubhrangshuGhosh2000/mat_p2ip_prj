...
from proc.benchmark_algo.li20.li20_mixed.LiDeepNetwork_mixed_rand20 import LiDeepNetworkModule
import time
...
from proc.benchmark_algo.li20.li20_mixed import RunTrainTest_mixed_Rand20
...
baseResultsFolderName = os.path.join(root_path, 'dataset/proc_data/benchmark_res/li20_res/li20_res_mixed/')
...
def RunAll():    
    if li2020Test:
        ...
        # if orgData:
        #     outResultsName = os.path.join(resultsFolderName, 'li20_res_mixed_OrigData.txt')
        ...
        #     outResultsName = os.path.join(resultsFolderName, 'li20_res_mixed_Rand50.txt')
        ...
        if HumanRandom20:
            ...
            outResultsNameLst = []
            outResultsNameLst.append(os.path.join(resultsFolderName, 'li20_res_mixed_Rand20_1.txt'))
            outResultsNameLst.append(os.path.join(resultsFolderName, 'li20_res_mixed_Rand20_2.txt'))
            # RunTrainTest_mixed_Rand20.runTestLst(LiDeepNetworkModule, outResultsNameLst,trainSets,testSets,folderName,hyp,resultsAppend=True, modelsLst=None,saveModels=convertToFolder(saves),predictionsFLst = pfs,startIdx=0)
            RunTrainTest_mixed_Rand20.runTestLst(LiDeepNetworkModule, outResultsNameLst,trainSets,testSets,folderName,hyp,resultsAppend=True, modelsLst=None,saveModels=None,predictionsFLst = pfs,startIdx=0)
            RunTrainTest_mixed_Rand20.calcOverallScore_Pos20(outResultsNameLst)

        # if HumanHeldOut50:
        #     trainSets, testSets, saves, pfs, folderName = loadHumanHeldOut50(resultsFolderName)
        #     outResultsName = os.path.join(resultsFolderName, 'li20_res_mixed_Held50.txt')
        #     runTest(LiDeepNetworkModule, outResultsName,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=convertToFolder(saves),predictionsFLst = pfs)
        #     calcOverallScore_Pos50(outResultsName)
        ...
        #     outResultsNameLst = []
        #     outResultsNameLst.append(os.path.join(resultsFolderName, 'li20_res_mixed_Held20_1.txt'))
        #     outResultsNameLst.append(os.path.join(resultsFolderName, 'li20_res_mixed_Held20_2.txt'))
