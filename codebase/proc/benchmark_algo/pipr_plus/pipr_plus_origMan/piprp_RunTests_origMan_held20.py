...
from proc.benchmark_algo.pipr_plus.pipr_plus_origMan.piprp_ChenNetwork_origMan_held20 import ChenNetworkModule
from utils.ProjectDataLoader import *
from utils.feat_engg_manual_main import extract_prot_seq_2D_manual_feat
from proc.benchmark_algo.pipr_plus.pipr_plus_origMan import piprp_RunTrainTest_origMan_held20
...
baseResultsFolderName = os.path.join(root_path, 'dataset/proc_data/benchmark_res/piprp_res/piprp_res_origMan/')
...
def RunAll():    
    if chen2019RNN:
        ...
        # if orgData:
        #     outResultsName = os.path.join(resultsFolderName, 'piprp_res_origMan_OrigData.txt')
        ...
        #     outResultsName = os.path.join(resultsFolderName, 'piprp_res_origMan_Rand50.txt')
        #     # piprp_RunTrainTest_origMan_Rand50.runTest(ChenNetworkModule, outResultsName,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=convertToFolder(saves),predictionsFLst = pfs,startIdx=0)
        #     piprp_RunTrainTest_origMan_rand50.runTest(ChenNetworkModule, outResultsName,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=None,predictionsFLst = pfs,startIdx=0)
        #     piprp_RunTrainTest_origMan_rand50.calcOverallScore_Pos50(outResultsName)

        # if HumanRandom20:
        #     trainSets, testSets, saves, pfs, folderName = loadHumanRandom20(resultsFolderName)
        #     outResultsNameLst = []
        #     outResultsNameLst.append(os.path.join(resultsFolderName, 'piprp_res_origMan_Rand20_1.txt'))
        #     outResultsNameLst.append(os.path.join(resultsFolderName, 'piprp_res_origMan_Rand20_2.txt'))
        ...
        #     outResultsName = os.path.join(resultsFolderName, 'piprp_res_origMan_Held50.txt')
        #     # pipr_RunTrainTest_origMan_held50.runTest(ChenNetworkModule, outResultsName,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=convertToFolder(saves),predictionsFLst = pfs,startIdx=0)
        #     piprp_RunTrainTest_origMan_held50.runTest(ChenNetworkModule, outResultsName,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=None,predictionsFLst = pfs,startIdx=0)
        #     piprp_RunTrainTest_origMan_held50.calcOverallScore_Pos50(outResultsName)

        if HumanHeldOut20:
            trainSets, testSets, saves, pfs, folderName = loadHumanHeldOut20(resultsFolderName)
            outResultsNameLst = []
            outResultsNameLst.append(os.path.join(resultsFolderName, 'piprp_res_origMan_Held20_1.txt'))
            outResultsNameLst.append(os.path.join(resultsFolderName, 'piprp_res_origMan_Held20_2.txt'))
            # piprp_RunTrainTest_origMan_held20.runTestLst(ChenNetworkModule, outResultsNameLst,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=convertToFolder(saves),predictionsFLst = pfs,startIdx=0)
            piprp_RunTrainTest_origMan_held20.runTestLst(ChenNetworkModule, outResultsNameLst,trainSets,testSets,folderName,hyp,resultsAppend=True,saveModels=None,predictionsFLst = pfs,startIdx=0)
            piprp_RunTrainTest_origMan_held20.calcOverallScore_Pos20(outResultsNameLst)
