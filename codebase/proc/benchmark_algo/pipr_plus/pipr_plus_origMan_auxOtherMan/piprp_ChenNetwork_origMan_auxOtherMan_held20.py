...
class ChenNetwork(nn.Module):
    def __init__(self,hiddenSize=50,inSize=14,aux_oneDencodingsize=1024,numLayers=6,n_heads=2,layer_1_size=1024,seed=1,fullGPU=False,deviceType='cpu'):
        ...
        self.linear1 = nn.Linear(2*(hiddenSize)+(2*hiddenSize), layer_1_size)  # "2*(hiddenSize+tl_1d_tensor_len)+(2*hiddenSize)" has the
    ...
    def forward(self,x):
        ...
        x = torch.cat((protA, other_man_1d_tensor, protB), dim=1)  # side-by-side concatenation 
