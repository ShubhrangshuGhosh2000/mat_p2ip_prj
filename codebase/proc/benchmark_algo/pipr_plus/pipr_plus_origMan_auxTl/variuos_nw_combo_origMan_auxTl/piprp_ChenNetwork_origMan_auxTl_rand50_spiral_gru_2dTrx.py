...
class ChenNetwork(nn.Module):
    def __init__(self,hiddenSize=50,inSize=14,aux_oneDencodingsize=1024,numLayers=6,seed=1,fullGPU=False,deviceType='cpu'):
        ...
        for i in range(0,self.numLayers):
            if(i == 0):  # first layer: only conv layer and no gru layer
                ...
                self.GRULst.append(None)  # no GRU for the first layer
            elif(i == 1):
            ...
            elif((i > 1) and (i <= self.numLayers-2)):  # intermediate layers between first and last layer : both conv and gru layers
                self.convLst.append(nn.Conv1d(hiddenSize*3,hiddenSize,3))
                self.GRULst.append(nn.GRU(input_size=hiddenSize*3,hidden_size=hiddenSize,bidirectional=True,batch_first=True))
            elif(i == self.numLayers -1):  # last layer: only conv layer and no gru layer
                self.convLst.append(nn.Conv1d(hiddenSize*3,hiddenSize*3,3))
                self.GRULst.append(None)  # no GRU for the last layer
        # end of for loop: for i in range(0,self.numLayers):


        self.both_prot_pos_enc = PositionalEncoder(d_model=hiddenSize*3, dropout=dropout_pos_enc, max_seq_len=700, batch_first=True)
        self.both_prot_transf_enc = nn.TransformerEncoder(encoder_layer=nn.TransformerEncoderLayer(d_model=hiddenSize*3, nhead=n_heads, dim_feedforward = hiddenSize, batch_first=True)
        ...
        layer_2_size = 512
        layer_3_size = 256
        self.linear1 = nn.Linear((3*hiddenSize+2*aux_oneDencodingsize), layer_1_size)
    ...
    def forward(self,x):
        ...
        for item in [protA, protB]: #run each protein through gru/pooling layers
            for i in range(0,self.numLayers):
                ...
                # if item is (a,b,c), then input to conv is (a,c,b) and input to gru is (a,b,c)
                if(i == 0):   # first layer: only conv layer and no gru layer
                    item = item.permute(0,2,1)  # item: (a,c,b)
                    item = self.convLst[i](item)
                    item = self.pooling(item)
                    item = item.permute(0,2,1)  # item: (a,b,c)
                    # print("From model.forward(): Size of item when i==0: {}".format(item.size()))
                elif((i >= 1) and (i <= self.numLayers-2)):  # intermediate layers between first and last layer : both conv and gru layers
                    ...
                    item_gru,hidden = self.GRULst[i](item)  # item: (a,b,c); item_gru: (a,b,c)
