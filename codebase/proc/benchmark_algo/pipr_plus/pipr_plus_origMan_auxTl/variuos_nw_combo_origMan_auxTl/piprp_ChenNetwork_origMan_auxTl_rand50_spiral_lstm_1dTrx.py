...
class ChenNetwork(nn.Module):
    def __init__(self,hiddenSize=50,inSize=14,aux_oneDencodingsize=1024,numLayers=6,seed=1,fullGPU=False,deviceType='cpu'):
        ...
        self.LSTMLst = nn.ModuleList()
        for i in range(0,self.numLayers):
            if(i == 0):  # first layer: only conv layer and no lstm layer
                ...
                self.LSTMLst.append(None)  # no LSTM for the first layer
            elif(i == 1):
                self.convLst.append(nn.Conv1d(hiddenSize,hiddenSize,3))
                self.LSTMLst.append(nn.LSTM(input_size=hiddenSize,hidden_size=hiddenSize,bidirectional=True,batch_first=True))
            elif((i > 1) and (i <= self.numLayers-2)):  # intermediate layers between first and last layer : both conv and lstm layers
                self.convLst.append(nn.Conv1d(hiddenSize*3,hiddenSize,3))
                self.LSTMLst.append(nn.LSTM(input_size=hiddenSize*3,hidden_size=hiddenSize,bidirectional=True,batch_first=True))
            elif(i == self.numLayers -1):  # last layer: only conv layer and no lstm layer
                self.convLst.append(nn.Conv1d(hiddenSize*3,hiddenSize,3))
                self.LSTMLst.append(None)  # no LSTM for the last layer


        # ############################ Transformer encoder part -start ############################
        n_heads = 4  # The number of attention heads (aka parallel attention layers). dim_val must be divisible by this number. Default: 8
        d_model = 256  # This can be any value divisible by n_heads. 512 is used in the original transformer paper. Default 512
        d_model = d_model - (d_model % n_heads)  # to make d_model divisible by n_heads
        self.n_features = 1
        seq_length = 2 * hiddenSize
        dropout_pos_enc = 0.10
        n_encoder_layers = 3  # Number of times the encoder layer is stacked in the encoder. Default: 4
        dim_feedforward = 256 # the dimension of the feedforward network model (default=2048).
    ...
    def forward(self,x):
        ...
        protLst = []
        for item in [protA, protB]: #run each protein through lstm/pooling layers
            for i in range(0,self.numLayers):
                #conv1d and pooling expect hidden dim on 2nd axis (dim=1), lstm needs hidden dim on 3rd axis (dim=2)
                # if item is (a,b,c), then input to conv is (a,c,b) and input to lstm is (a,b,c)
                if(i == 0):   # first layer: only conv layer and no lstm layer
                    ...
                    item_lstm,hidden = self.LSTMLst[i](item)  # item: (a,b,c); item_lstm: (a,b,c)
                    # print("From model.forward(): Size of item_lstm when i is " + str(i) + " : " + str(item_lstm.size()))
                    item = torch.cat((item_conv,item_lstm[:, :item_conv.shape[1], :]),2)  # item_conv: (a,b,c); item_lstm: (a,b,c); item: (a,b,c);
                    # item = torch.cat((item_conv,self.perform_dim_reductn(item_lstm, item_conv.shape[1])),2)  # item_conv: (a,b,c); item_lstm: (a,b,c); item: (a,b,c);
                    ...
                    item = item.permute(0,2,1)  # item: (a,c,b)
                    item = self.convLst[i](item)
                    # print("From model.forward(): Size of item  at end when i is last layer: " + str(i) + " : " + str(item.size()))
