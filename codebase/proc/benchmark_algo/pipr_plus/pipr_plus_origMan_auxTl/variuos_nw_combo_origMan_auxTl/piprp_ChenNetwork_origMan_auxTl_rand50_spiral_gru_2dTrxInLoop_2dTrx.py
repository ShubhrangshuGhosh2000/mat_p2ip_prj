...
class ChenNetwork(nn.Module):
    def __init__(self,hiddenSize=50,inSize=14,aux_oneDencodingsize=1024,numLayers=6,seed=1,fullGPU=False,deviceType='cpu'):
        ...
        self.numLayers = 5 #  numLayers
        self.fullGPU = fullGPU
        self.deviceType = deviceType

        # ############################ Transformer encoder part -start ############################
        n_heads = 2  # The number of attention heads (aka parallel attention layers). dim_val must be divisible by this number. Default: 8
        ...
        self.both_prot_pos_enc = PositionalEncoder(d_model=hiddenSize, dropout=dropout_pos_enc, max_seq_len=700, batch_first=True)
        self.both_prot_transf_enc = nn.TransformerEncoder(encoder_layer=nn.TransformerEncoderLayer(d_model=hiddenSize, nhead=n_heads, dim_feedforward = hiddenSize, batch_first=True)
                                                        , num_layers=n_encoder_layers, norm=None)
        ...
        layer_2_size = 512
        layer_3_size = 256
        self.linear1 = nn.Linear(hiddenSize+(2*aux_oneDencodingsize), layer_1_size)
    ...
    def forward(self,x):
        ...
        for item in [protA, protB]: #run each protein through gru/pooling layers
            for i in range(0,self.numLayers):
                ...
                if(i == 0):   # 0th layer: only conv layer and no gru layer
                    ...
                    item_gru = item_gru.permute(0,2,1)  # item_gru: (a,c,b)
                    item_gru = self.pooling(item_gru[:, :, :item_gru.shape[2]-2])
                    item_gru = item_gru.permute(0,2,1)  # item_gru: (a,b,c)
                    # print("From model.forward(): Size of item_gru after pooling when i is " + str(i) + " : " + str(item_gru.size()))
                    item = torch.cat((item_conv,item_gru),2)  # item_conv: (a,b,c1); item_gru: (a,b,c2); item: (a,b,c1+c2);
                    # item = torch.cat((item_conv,item_gru[:, :item_conv.shape[1], :]),2)  # item_conv: (a,b,c1); item_gru: (a,b,c2); item: (a,b,c1+c2);
                    ...
                    item_trx = item_trx.permute(0,2,1)  # item_trx: (a,c,b)
                    item_trx = self.pooling(item_trx[:, :, :item_trx.shape[2]-2])
                    item_trx = item_trx.permute(0,2,1)  # item_trx: (a,b,c)
                    # print("From model.forward(): Size of item_trx after pooling when i is " + str(i) + " : " + str(item_gru.size()))
                    item = torch.cat((item_conv,item_trx),2)  # item_conv: (a,b,c); item_trx: (a,b,c); item: (a,b,c);
                    ...
                    item = item.permute(0,2,1)  # item: (a,b,c)
                    # print("From model.forward(): Size of item (a,b,c)  at end when i is last layer: " + str(i) + " : " + str(item.size()))
                    protLst.append(item)
            # end of for loop: for i in range(0,self.numLayers):
        ...
        # concatenate protA and protB over dim=1 (b of (a,b,c))
        concat_protA_protB = torch.cat((protA, protB), dim=1)
        # print("From model.forward(): Size of concat_protA_protB: " + str(concat_protA_protB.shape))
        # apply the transformer encoder on the concatenated sequence
        concat_protA_protB = self.both_prot_pos_enc(concat_protA_protB)
        concat_protA_protB = self.both_prot_transf_enc(concat_protA_protB)
        # print("From model.forward(): Size of concat_protA_protB after both_prot_transf_enc : " + str(concat_protA_protB.shape))
        concat_protA_protB = concat_protA_protB.mean(dim=1) # global average pooling over dim 1, reducing the data from 3D to 2D
        # print("From model.forward(): Size of concat_protA_protB after global average pooling over dim 1:" + " : " + str(concat_protA_protB.size()))

        # now horizontally concatenate, auxProtA and auxProtB with concat_protA_protB
        x = torch.cat((auxProtA, concat_protA_protB, auxProtB), dim=1)  # side-by-side concatenation 
