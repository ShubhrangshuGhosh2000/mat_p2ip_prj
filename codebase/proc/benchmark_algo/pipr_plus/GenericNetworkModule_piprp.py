...
class GenericNetworkModule(GenericModule):
    ...
    #when running fit or predict, pass in the dataMatrix
    def predict_proba_xai_humanBenchmark(self,predictFeatures,predictClasses,predictFileName):
        attrbn_df = self.model.predict_proba_xai_humanBenchmark(predictFeatures,self.dataMatrix, self.oneDdataMatrix, predictClasses, predictFileName)
        return attrbn_df
