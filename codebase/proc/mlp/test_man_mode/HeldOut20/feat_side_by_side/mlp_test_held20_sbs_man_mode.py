...
from proc.mlp.train_man_mode.HeldOut20.feat_side_by_side.mlp_train_held20_sbs_fixed_lr_man_mode import MlpModelHeld20Sbs_fixedLrTrain_ManMode

CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data_man_mode/HeldOut20/feat_side_by_side'
TEST_RES_DIR = 'dataset/proc_data/mlp_data/test_data_man_mode/HeldOut20/feat_side_by_side'
...
def load_final_ckpt_model(root_path='./', train_pkl_name = 'train.pkl'):
    ...
    final_ckpt_path = os.path.join(final_ckpt_dir, "MlpModelHeld20Sbs_fixedLrTrain_ManMode_" + str(train_pkl_name_without_ext) + '*.ckpt')
    final_ckpt_file_name = glob.glob(final_ckpt_path, recursive=False)[0]
    print('final_ckpt_file_name: ' + str(final_ckpt_file_name))
    # load the model
    model = MlpModelHeld20Sbs_fixedLrTrain_ManMode.load_from_checkpoint(final_ckpt_file_name)
    # # automatically restores model, epoch, step, LR schedulers, apex, etc...
    # model = MlpModelHeld20Sbs_fixedLrTrain()
    # trainer = pl.Trainer()
...
def prepare_test_data(root_path='./', train_pkl_name = 'train.pkl', test_pkl_name = 'test.pkl', model = None):
    ...
    manual_test_data_path = os.path.join(root_path, 'dataset/preproc_data/human_2021_manual/HeldOut20/feat_side_by_side', test_pkl_name)
    ...
    print('X_test_arr_manual.shape: ' + str(X_test_arr_manual.shape))  # (, 3708)

    # z-normalize X_test_arr_manual feature(column) wise using the scaler saved during the training data preparation
    X_test_scaled_reduced = model.scaler.transform(X_test_arr_manual)
    y_test_arr = y_test_arr_manual
    print('X_test_scaled_reduced.shape: ' + str(X_test_scaled_reduced.shape))  # (, 3708)
    ...
    return (test_arr_2d_manual, X_test, y_test)
...
def test_model(root_path='./', train_pkl_name = 'train.pkl', test_pkl_name = 'test.pkl'):
    ...
    file_nm_with_loc = os.path.join(root_path, TEST_RES_DIR
                                           , 'HeldOut20_' + test_pkl_name_without_ext + '_pred_res.csv')
    ...
    score_dict_pkl_name_with_loc = os.path.join(root_path, TEST_RES_DIR
                                           , 'HeldOut20_' + test_pkl_name_without_ext + '_score.pkl')
    ...
    score_df = pd.DataFrame({'dataset': ['Held20'], 'testset': [test_pkl_name_without_ext]
    ...
    score_df_name_with_loc = os.path.join(root_path, TEST_RES_DIR
                                           , 'HeldOut20_' + test_pkl_name_without_ext + '_score.csv')
    ...
    for test_index in range(1, 3):
        ...
        for first_ind in range(0, index_limit + 1):
            for second_ind in range(first_ind, index_limit + 1):
                test_pkl_name = 'Test' + str(test_index) + '_' + str(first_ind) + '_' + str(second_ind) + '.pkl'
                print('\ntest_pkl_name: ' + str(test_pkl_name))
                test_pkl_name_without_ext = test_pkl_name.replace('.pkl', '') 
                indiv_score_csv_file_nm = 'HeldOut20_' + str(test_pkl_name_without_ext) + '_score.csv'
        ...
        # end of for loop: for first_ind in range(0, index_limit + 1):
        ...
        # save con_score_df
        con_score_df.to_csv(os.path.join(root_path, TEST_RES_DIR, 'con_score_df_' +  str(test_index) + '.csv'), index=False)
