...
from proc.mlp.train_man_mode.Random50.feat_side_by_side.mlp_train_rand50_sbs_fixed_lr_man_mode import MlpModelRand50Sbs_fixedLrTrain_ManMode

CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data_man_mode/Random50/feat_side_by_side'
TEST_RES_DIR = 'dataset/proc_data/mlp_data/test_data_man_mode/Random50/feat_side_by_side'
...
def load_final_ckpt_model(root_path='./', train_pkl_name = 'train.pkl'):
    ...
    final_ckpt_path = os.path.join(final_ckpt_dir, "MlpModelRand50Sbs_fixedLrTrain_ManMode_" + str(train_pkl_name_without_ext) + '*.ckpt')
    final_ckpt_file_name = glob.glob(final_ckpt_path, recursive=False)[0]
    print('final_ckpt_file_name: ' + str(final_ckpt_file_name))
    # load the model
    model = MlpModelRand50Sbs_fixedLrTrain_ManMode.load_from_checkpoint(final_ckpt_file_name)
    # # automatically restores model, epoch, step, LR schedulers, apex, etc...
    # model = MlpModelRand50Sbs_fixedLrTrain_ManMode()
    # trainer = pl.Trainer()
...
def prepare_test_data(root_path='./', train_pkl_name = 'train.pkl', test_pkl_name = 'test.pkl', model = None):
    ...
    manual_test_data_path = os.path.join(root_path, 'dataset/preproc_data/human_2021_manual/Random50/feat_side_by_side', test_pkl_name)
