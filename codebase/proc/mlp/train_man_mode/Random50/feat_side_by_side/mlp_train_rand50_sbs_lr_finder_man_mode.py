...
CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data_man_mode/Random50/feat_side_by_side'
thresholds = [0.03, 1.0]

# Multilayer Perceptron (MLP) Model for the Side-By-Side(sbs) features using Random50 training sets
class MlpModelRand50Sbs_lrFinder_ManMode(pl.LightningModule):
...
# ################################## VERY IMPORTANT NOTES -END #######################
    ...
    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelRand50Sbs_lrFinder_ManMode, self).__init__()
    ...
    def prepare_data(self):
        ...
        # # z-normalize X_val_arr_manual feature(column) wise
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10, lr_finder=False):
    ...
    model = MlpModelRand50Sbs_lrFinder_ManMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelRand50Sbs_lrFinder_ManMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
