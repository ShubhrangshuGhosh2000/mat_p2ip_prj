...
from proc.mlp.train_man_mode.HeldOut20.feat_side_by_side.mlp_train_held20_sbs_lr_finder_man_mode import MlpModelHeld20Sbs_lrFinder_ManMode
...
class MlpModelHeld20Sbs_fixedLrTrain_ManMode(MlpModelHeld20Sbs_lrFinder_ManMode):
...
# ################################## VERY IMPORTANT NOTES -END #######################
    ...
    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelHeld20Sbs_fixedLrTrain_ManMode, self).__init__(config, root_path, result_dir, pkl_name)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10):
    ...
    model = MlpModelHeld20Sbs_fixedLrTrain_ManMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelHeld20Sbs_fixedLrTrain_ManMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
