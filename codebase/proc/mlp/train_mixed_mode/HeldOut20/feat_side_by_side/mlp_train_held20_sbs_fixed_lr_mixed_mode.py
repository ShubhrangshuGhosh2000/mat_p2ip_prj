...
from proc.mlp.train_mixed_mode.HeldOut20.feat_side_by_side.mlp_train_held20_sbs_lr_finder_mixed_mode import MlpModelHeld20Sbs_lrFinder_MixedMode
from utils import dl_reproducible_result_util

CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data_mixed_mode/HeldOut20/feat_side_by_side'

# Multilayer Perceptron (MLP) Model for the Side-By-Side(sbs) features using Randim20 training sets
class MlpModelHeld20Sbs_fixedLrTrain_MixedMode(MlpModelHeld20Sbs_lrFinder_MixedMode):
...
# ################################## VERY IMPORTANT NOTES -END #######################
    ...
    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelHeld20Sbs_fixedLrTrain_MixedMode, self).__init__(config, root_path, result_dir, pkl_name)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10):
    ...
    model = MlpModelHeld20Sbs_fixedLrTrain_MixedMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelHeld20Sbs_fixedLrTrain_MixedMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
    ...
    early_stop_callback = pl.callbacks.early_stopping.EarlyStopping(
        ...
        monitor="train/val_prec", mode="max"
        , min_delta=0.00, patience=5000, verbose=True)  # monitor="val_prec" can be checked
...
def start(root_path='./'):
    ...
    # TEMP CODE -START
    # train_file_name_lst = [trn_fl_nm for trn_fl_nm in train_file_name_lst if trn_fl_nm not in ['Train_0_0.pkl', 'Train_0_1.pkl']]
    # train_file_name_lst = [trn_fl_nm for trn_fl_nm in train_file_name_lst if(trn_fl_nm.startswith('Train_5') and trn_fl_nm not in ['Train_1_1.pkl', 'Train_1_2.pkl'])]
    # TEMP CODE -END
