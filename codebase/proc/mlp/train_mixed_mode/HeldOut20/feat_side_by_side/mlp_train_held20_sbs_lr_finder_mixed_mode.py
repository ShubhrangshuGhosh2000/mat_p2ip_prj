...
CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data_mixed_mode/HeldOut20/feat_side_by_side'
thresholds = [0.03, 1.0]

# Multilayer Perceptron (MLP) Model for the Side-By-Side(sbs) features using HeldOut20 training sets
class MlpModelHeld20Sbs_lrFinder_MixedMode(pl.LightningModule):
...
# ################################## VERY IMPORTANT NOTES -END #######################
    ...
    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelHeld20Sbs_lrFinder_MixedMode, self).__init__()
    ...
    def prepare_data(self):
        ...
        tl_training_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021/HeldOut20/feat_side_by_side', self.pkl_name)
        ...
        #     val_data_path_tl = os.path.join(self.root_path, 'dataset/preproc_data/human_2021/HeldOut20/feat_side_by_side', val_pkl_name_tl)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10, lr_finder=False):
    ...
    model = MlpModelHeld20Sbs_lrFinder_MixedMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelHeld20Sbs_lrFinder_MixedMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
