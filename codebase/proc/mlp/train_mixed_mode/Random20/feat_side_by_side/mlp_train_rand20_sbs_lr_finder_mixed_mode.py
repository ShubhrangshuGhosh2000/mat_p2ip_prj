...
# Multilayer Perceptron (MLP) Model for the Side-By-Side(sbs) features using Random20 training sets
class MlpModelRand20Sbs_lrFinder_MixedMode(pl.LightningModule):
...
# ################################## VERY IMPORTANT NOTES -END #######################
    ...
    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelRand20Sbs_lrFinder_MixedMode, self).__init__()
    ...
    def prec_spl(self, logits, labels):
        ...
        prec = score_dict['Max Precision'][0]  # precision at 3% recall
        return torch.tensor(prec)
    ...
    def prepare_data(self):
        ...
        tl_training_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021/Random20/feat_side_by_side', self.pkl_name)
        ...
        manual_training_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021_manual/Random20/feat_side_by_side', self.pkl_name)
        ...
        # val_lst_tl = []
        # for i in range(1, 3):
        #     val_pkl_name_tl = self.pkl_name.replace('Train', 'Test' + str(i))
        #     val_data_path_tl = os.path.join(self.root_path, 'dataset/preproc_data/human_2021/Random20/feat_side_by_side', val_pkl_name_tl)
        #     val_lst_tl += joblib.load(val_data_path_tl)
        # val_arr_2d_tl = np.vstack(val_lst_tl)
        # val_arr_tl = val_arr_2d_tl[:, list(range(3, val_arr_2d_tl.shape[1])) + [2]]
        # X_val_arr_tl = val_arr_tl[:, range(0, val_arr_tl.shape[1] -1)]
        # y_val_arr_tl = val_arr_tl[:, -1]

        # val_lst_manual = []
        # for i in range(1, 3):
        #     val_pkl_name_manual = self.pkl_name.replace('Train', 'Test' + str(i))
        #     val_data_path_manual = os.path.join(self.root_path, 'dataset/preproc_data/human_2021_manual/Random20/feat_side_by_side', val_pkl_name_manual)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10, lr_finder=False):
    ...
    model = MlpModelRand20Sbs_lrFinder_MixedMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelRand20Sbs_lrFinder_MixedMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
    ...
    # , name="", prefix='', tags=["training-random20-sbs"]
    # )
...
def generate_train_test_df_csv(root_path='./'):
    ...
    # for ind in range(0, 1):
        ...
        test1_df_path = os.path.join(result_dir, 'test1_' + str(ind) + '.csv')
        test1_df.to_csv(test1_df_path, index=False)
        test2_df_path = os.path.join(result_dir, 'test2_' + str(ind) + '.csv')
