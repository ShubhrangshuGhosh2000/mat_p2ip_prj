...
# Multilayer Perceptron (MLP) Model for the Side-By-Side(sbs) features using HeldOut50 training sets
class MlpModelHeld50Sbs_lrFinder_MixedMode(pl.LightningModule):
...
# ################################## VERY IMPORTANT NOTES -END #######################
    ...
    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelHeld50Sbs_lrFinder_MixedMode, self).__init__()
    ...
    def prepare_data(self):
        ...
        tl_training_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021/HeldOut50/feat_side_by_side', self.pkl_name)
        ...
        manual_training_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021_manual/HeldOut50/feat_side_by_side', self.pkl_name)
        ...
        print('X_train_scaled_reduced.shape: ' + str(X_train_scaled_reduced.shape))  # (, 5756)
        ...
        # tl_val_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021/HeldOut50/feat_side_by_side', val_pkl_name)
        ...
        # manual_val_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021_manual/HeldOut50/feat_side_by_side', val_pkl_name)
        ...
        # # z-normalize X_val_concatenated feature(column) wise
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10, lr_finder=False):
    ...
    model = MlpModelHeld50Sbs_lrFinder_MixedMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelHeld50Sbs_lrFinder_MixedMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
