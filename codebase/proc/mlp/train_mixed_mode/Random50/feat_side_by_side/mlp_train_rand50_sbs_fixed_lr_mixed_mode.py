...
from proc.mlp.train_mixed_mode.Random50.feat_side_by_side.mlp_train_rand50_sbs_lr_finder_mixed_mode import MlpModelRand50Sbs_lrFinder_MixedMode
...
class MlpModelRand50Sbs_fixedLrTrain_MixedMode(MlpModelRand50Sbs_lrFinder_MixedMode):
...
# ################################## VERY IMPORTANT NOTES -END #######################
    ...
    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelRand50Sbs_fixedLrTrain_MixedMode, self).__init__(config, root_path, result_dir, pkl_name)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10):
    ...
    model = MlpModelRand50Sbs_fixedLrTrain_MixedMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelRand50Sbs_fixedLrTrain_MixedMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
