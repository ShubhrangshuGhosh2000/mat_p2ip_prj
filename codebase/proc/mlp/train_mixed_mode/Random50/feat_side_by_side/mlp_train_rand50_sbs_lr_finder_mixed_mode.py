...
CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data_mixed_mode/Random50/feat_side_by_side'
thresholds = [0.03, 1.0]

# Multilayer Perceptron (MLP) Model for the Side-By-Side(sbs) features using Random50 training sets
class MlpModelRand50Sbs_lrFinder_MixedMode(pl.LightningModule):
...
# ################################## VERY IMPORTANT NOTES -END #######################
    ...
    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelRand50Sbs_lrFinder_MixedMode, self).__init__()
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        # accuracy = self.accuracy_spl(logits, y)
    ...
    def prepare_data(self):
        ...
        tl_training_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021/Random50/feat_side_by_side', self.pkl_name)
        ...
        print('X_train_arr_tl.shape: ' + str(X_train_arr_tl.shape))  # (, 2048)

        # extract the manual features for the training data
        print('\nExtracting the manual features for the training data...')
        manual_training_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021_manual/Random50/feat_side_by_side', self.pkl_name)
        ...
        print('X_train_arr_manual.shape: ' + str(X_train_arr_manual.shape))  # (, 2436)
        ...
        X_train_concatenated = np.concatenate((X_train_arr_tl, X_train_arr_manual), axis=1)  # column-wise concatenation
        # z-normalize X_train_concatenated feature(column) wise
        scaler = preprocessing.StandardScaler()
        X_train_scaled_reduced = scaler.fit_transform(X_train_concatenated)
        y_train_arr = y_train_arr_tl
        print('X_train_scaled_reduced.shape: ' + str(X_train_scaled_reduced.shape))  # (, 4484)
        ...
        # tl_val_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021/Random50/feat_side_by_side', val_pkl_name)
        ...
        # manual_val_data_path = os.path.join(self.root_path, 'dataset/preproc_data/human_2021_manual/Random50/feat_side_by_side', val_pkl_name)
        # val_lst_manual = joblib.load(manual_val_data_path)
        # val_arr_2d_manual = np.vstack(val_lst_manual)
        # val_arr_manual = val_arr_2d_manual[:, list(range(3, val_arr_2d_manual.shape[1])) + [2]]
        # X_val_arr_manual = val_arr_manual[:, range(0, val_arr_manual.shape[1] -1)]  # excluding the target column
        # y_val_arr_manual = val_arr_manual[:, -1]  # the last column i.e. target column

        # X_val_concatenated = np.concatenate((X_val_arr_tl, X_val_arr_manual), axis=1)  # column-wise concatenation
        # X_val_scaled_reduced = scaler.transform(X_val_concatenated)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10, lr_finder=False):
    ...
    model = MlpModelRand50Sbs_lrFinder_MixedMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelRand50Sbs_lrFinder_MixedMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
    + "-batch" + str(config["batch_size"]) + "-dim" + str(2 * config["reduc_dim"]) + "-lr{:0.4f}-".format(config["lr"])
...
def generate_train_test_df_csv(root_path='./'):
    ...
    # for ind in range(0, 1):
        ...
        X_test_scaled_reduced = part_preproc_dict['X_test_scaled_reduced']
        y_test_arr = part_preproc_dict['y_test_arr']
        ...
        test_df = pd.DataFrame(data=X_test_scaled_reduced, columns=col_nm_lst)
        test_df['target'] = y_test_arr
        train_df_path = os.path.join(result_dir, 'train_' + str(ind) + '.csv')
        train_df.to_csv(train_df_path, index=False)
        test_df_path = os.path.join(result_dir, 'test_' + str(ind) + '.csv')
...
def start(root_path='./'):
    ...
    # for ind in range(0, 1):
        ...
        if os.path.exists(best_config_pkl_name_with_path):
            ...
            , "reduc_dim": 5756  # 5756  # (1024 + 1024) + (1854 + 1854) using Li2020 + Jia2019 + Du2017
