...
CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data/Random50/feat_post_seq_concat'

# Multilayer Perceptron (MLP) Model for the Post-Sequence-Concatenation(psc) features using Randim50 training sets
class MlpModelRand50Psc(pl.LightningModule):

    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelRand50Psc, self).__init__()
        ...
        # for side-by-side (psc), orginal feature vector length is 2048 (1024 for 
    ...
    def prepare_data(self):
        ...
        'dataset/proc_data/mlp_data/training_data/Random50/feat_post_seq_concat', pkl_name_without_ext + '_part_preproc_dim_' + \
        ...
        if os.path.exists(part_preproc_pkl_path):
            ...
            'dataset/preproc_data/human_2021/Random50/feat_post_seq_concat', self.pkl_name)
            ...
            'dataset/preproc_data/human_2021/Random50/feat_post_seq_concat', val_pkl_name)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10):
    ...
    model = MlpModelRand50Psc(config, root_path, result_dir, pkl_name)

    # define the checkpoint_callback
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
                        dirpath=result_dir
                        , filename="MlpModelRand50Psc-" + str(pkl_name_without_ext) + "-epoch{epoch:02d}-val_accuracy{train/val_accuracy:.2f}"
...
def start(root_path='./'):
    ...
    for ind in range(0, 1):
        ...
        num_samples = 100
        num_epochs = 50
        result_dir = os.path.join(root_path, 'dataset/proc_data/mlp_data/training_data/Random50/feat_post_seq_concat',
        ...
        # mlp_train_rand50_psc_final.start(root_path, pkl_name)

        # perform the testing 
        print('\n# ############################# performing the testing for ' + pkl_name + ' #############################\n')
        # mlp_test_rand50_psc.start(root_path, pkl_name)
    # end of for loop
