...
from proc.mlp.train_tl_mode.Random50.feat_post_seq_concat.mlp_train_rand50_psc_lr_finder_tl_mode import MlpModelRand50Psc_lrFinder_TlMode
...
class MlpModelRand50Psc_fixedLrTrain_TlMode(MlpModelRand50Psc_lrFinder_TlMode):

    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelRand50Psc_fixedLrTrain_TlMode, self).__init__(config, root_path, result_dir, pkl_name)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10):
    ...
    model = MlpModelRand50Psc_fixedLrTrain_TlMode(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelRand50Psc_fixedLrTrain_TlMode_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
