...
from codebase.proc.mlp.train.Random50.feat_side_by_side import mlp_train_rand50_sbs_final
from codebase.proc.mlp.test.Random50.feat_side_by_side import mlp_test_rand50_sbs
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10):
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
                        dirpath=result_dir
                        , filename="MlpModelRand50Sbs-" + str(pkl_name_without_ext) + "-epoch{epoch:02d}-val_loss{train/val_loss:.2f}"
                        , auto_insert_metric_name=False
                        , save_top_k=1, monitor='train/val_loss', mode='min', every_n_epochs = 1, verbose=True)

     # define the early_stop_callback
    early_stop_callback = pl.callbacks.early_stopping.EarlyStopping(
        monitor="train/val_loss", mode="min", min_delta=0.00, patience=3, verbose=True)  # monitor="val_acc" can be checked
    ...
    tune_report_chkpt_callback = TuneReportCheckpointCallback(
                            metrics={
                                "loss": "train/val_loss",
                                "mean_accuracy": "train/val_accuracy"
                            },
                            filename="checkpoint",
                            on="validation_end")
...
def start(root_path='./'):
    ...
    for ind in range(0, 1):
        ...
        # config = {
            ...
            # , "reduc_dim": tune.grid_search([5, 10, 25])  # for host109
        # }
