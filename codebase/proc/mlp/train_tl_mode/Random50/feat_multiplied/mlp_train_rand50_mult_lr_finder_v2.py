...
CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data/Random50/feat_multiplied'
INPUT_DIR = 'dataset/preproc_data/human_2021/Random50/feat_multiplied'
thresholds = [0.03, 1.0]

# Multilayer Perceptron (MLP) Model for the multiplied(mult) features using Randim50 training sets
class MlpModelRand50Mult_lrFinder(pl.LightningModule):

    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelRand50Mult_lrFinder, self).__init__()
        ...
        # for multiplied (mult), orginal feature vector length is 2048 (1024 for 
    ...
    def prepare_data(self):
        ...
        if os.path.exists(part_preproc_pkl_path):
            ...
            training_data_path = os.path.join(self.root_path, INPUT_DIR, self.pkl_name)
            ...
            val_data_path = os.path.join(self.root_path, INPUT_DIR, val_pkl_name)
            ...
            test_data_path = os.path.join(self.root_path, INPUT_DIR, test_pkl_name)
            # load the pkl file
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10, epoch_finder=True, lr_finder=False):
    ...
    model = MlpModelRand50Mult_lrFinder(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelRand50Mult_lrFinder_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
    ...
    # , name="", prefix='', tags=["training-random50-mult"]
    # )
