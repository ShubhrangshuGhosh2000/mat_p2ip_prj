...
from codebase.proc.mlp.train.Random50.feat_multiplied.mlp_train_rand50_mult_lr_finder_v2 import MlpModelRand50Mult_lrFinder
...
class MlpModelRand50Mult_fixedLrTrain(MlpModelRand50Mult_lrFinder):

    def __init__(self, config = None, root_path='./', result_dir='./', pkl_name = 'Train.pkl'):
        super(MlpModelRand50Mult_fixedLrTrain, self).__init__(config, root_path, result_dir, pkl_name)
...
def train_model(config = None, root_path='./', result_dir='./', pkl_name = 'train.pkl', num_epochs = 10):
    ...
    model = MlpModelRand50Mult_fixedLrTrain(config, root_path, result_dir, pkl_name)
    pkl_name_without_ext = pkl_name.replace('.pkl', '') 
    ckpt_file_nm_1st_part = "MlpModelRand50Mult_fixedLrTrain_" + pkl_name_without_ext + '-' + str(config["layer_1_size"]) + "-" + str(config["layer_2_size"]) + "-" + str(config["layer_3_size"]) \
