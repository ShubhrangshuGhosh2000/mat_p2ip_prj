...
from proc.mlp.train_tl_mode.Random20.feat_side_by_side.mlp_train_rand20_sbs_fixed_lr_tl_mode import MlpModelRand20Sbs_fixedLrTrain_TlMode

CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data_tl_mode/Random20/feat_side_by_side'
TEST_RES_DIR = 'dataset/proc_data/mlp_data/test_data_tl_mode/Random20/feat_side_by_side'
...
def load_final_ckpt_model(root_path='./', train_pkl_name = 'train.pkl'):
    ...
    final_ckpt_path = os.path.join(final_ckpt_dir, "MlpModelRand20Sbs_fixedLrTrain_TlMode_" + str(train_pkl_name_without_ext) + '*.ckpt')
    final_ckpt_file_name = glob.glob(final_ckpt_path, recursive=False)[0]
    print('final_ckpt_file_name: ' + str(final_ckpt_file_name))
    # load the model
    model = MlpModelRand20Sbs_fixedLrTrain_TlMode.load_from_checkpoint(final_ckpt_file_name)
    # # automatically restores model, epoch, step, LR schedulers, apex, etc...
    # model = MlpModelRand20Sbs_fixedLrTrain_TlMode()
    # trainer = pl.Trainer()
