...
from codebase.proc.mlp.train.Random50.feat_multiplied.mlp_train_rand50_mult_fixed_lr_train_v2 import MlpModelRand50Mult_fixedLrTrain

CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data/Random50/feat_multiplied'
TEST_RES_DIR = 'dataset/proc_data/mlp_data/test_data/Random50/feat_multiplied'
...
def load_final_ckpt_model(root_path='./', train_pkl_name = 'train.pkl'):
    ...
    final_ckpt_path = os.path.join(final_ckpt_dir, "MlpModelRand50Mult_fixedLrTrain_" + str(train_pkl_name_without_ext) + '*.ckpt')
    final_ckpt_file_name = glob.glob(final_ckpt_path, recursive=False)[0]
    print('final_ckpt_file_name: ' + str(final_ckpt_file_name))
    # load the model
    model = MlpModelRand50Mult_fixedLrTrain.load_from_checkpoint(final_ckpt_file_name)
    # # automatically restores model, epoch, step, LR schedulers, apex, etc...
    # model = MlpModelRand50Mult_fixedLrTrain()
    # trainer = pl.Trainer()
...
def prepare_test_data(root_path='./', train_pkl_name = 'train.pkl', test_pkl_name = 'test.pkl', model = None):
    ...
    if os.path.exists(test_part_preproc_pkl_path):
        ...
        'dataset/preproc_data/human_2021/Random50/feat_multiplied', test_pkl_name)
