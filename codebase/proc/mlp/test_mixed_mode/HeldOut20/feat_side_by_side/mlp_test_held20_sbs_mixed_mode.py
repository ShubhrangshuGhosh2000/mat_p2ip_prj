...
from proc.mlp.train_mixed_mode.HeldOut20.feat_side_by_side.mlp_train_held20_sbs_fixed_lr_mixed_mode import MlpModelHeld20Sbs_fixedLrTrain_MixedMode

CHECKPOINTS_DIR = 'dataset/proc_data/mlp_data/training_data_mixed_mode/HeldOut20/feat_side_by_side'
TEST_RES_DIR = 'dataset/proc_data/mlp_data/test_data_mixed_mode/HeldOut20/feat_side_by_side'
...
def load_final_ckpt_model(root_path='./', train_pkl_name = 'train.pkl'):
    ...
    final_ckpt_path = os.path.join(final_ckpt_dir, "MlpModelHeld20Sbs_fixedLrTrain_MixedMode_" + str(train_pkl_name_without_ext) + '*.ckpt')
    final_ckpt_file_name = glob.glob(final_ckpt_path, recursive=False)[0]
    print('final_ckpt_file_name: ' + str(final_ckpt_file_name))
    # load the model
    model = MlpModelHeld20Sbs_fixedLrTrain_MixedMode.load_from_checkpoint(final_ckpt_file_name)
...
def prepare_test_data(root_path='./', train_pkl_name = 'train.pkl', test_pkl_name = 'test.pkl', model = None):
    ...
    tl_test_data_path = os.path.join(root_path, 'dataset/preproc_data/human_2021/HeldOut20/feat_side_by_side', test_pkl_name)
