...
class ChenNetwork(nn.Module):
    def __init__(self,hiddenSize=50,inSize=14,aux_oneDencodingsize=1024,numLayers=6,seed=1,fullGPU=False,deviceType='cpu'):
        ...
        # for i in range(0,self.numLayers):
        for i in range(0,self.numLayers-3):  # range(0,3)
            if i == 0: #first convolutions takes data of input size, other 5 take data of hidden size * 3
                self.otherManConvLst.append(nn.Conv1d(in_channels,hiddenSize*3,3))
            else:
                self.otherManConvLst.append(nn.Conv1d(hiddenSize*3,hiddenSize*3,3))
        ...
        layer_2_size = 512
        layer_3_size = 256
        self.linear1 = nn.Linear(2*(hiddenSize+tl_1d_tensor_len)+(3*hiddenSize), layer_1_size)  # "2*(hiddenSize+tl_1d_tensor_len)+(2*hiddenSize)" has the
    ...
    def forward(self,x):
        ...
        for i in range(0,self.numLayers-4):  # range(0,2)
            ...
            other_man_2d_tensor = other_man_2d_tensor.permute(0,2,1)
        other_man_2d_tensor = other_man_2d_tensor.permute(0,2,1)
        other_man_2d_tensor = self.otherManConvLst[self.numLayers-4](other_man_2d_tensor)
