...
# prepare the dataset by the feature intermixing for the given dataset type
def data_prep_feat_intermixed(root_path='./', dataset_type='Random50'):
    print("\n ############ inside the data_prep_feat_intermixed() method ############")
    input_path = os.path.join(root_path, 'dataset/orig_data/Human2021/BioGRID2021', dataset_type)
    output_path = os.path.join(root_path, 'dataset/preproc_data/human_2021', dataset_type, 'feat_intermixed')
    ...
    # iterate over the tsv files and perform the feature intermixing
    for indiv_tsv_file_nm in tsv_file_names_lst:
        ...
        for ind, row in indiv_tsv_df.iterrows():
            ...
            prot_1_id = row[0]
            prot_2_id = row[1]
            prot_1_feat_lst = human_seq_feat_dict[prot_1_id]['seq_feat'].tolist()
            prot_2_feat_lst = human_seq_feat_dict[prot_2_id]['seq_feat'].tolist()
            # intermix the feature list
            feat_intermixed_lst = []
            for i in range(len(prot_1_feat_lst)):
                feat_intermixed_lst.append(prot_1_feat_lst[i])
                feat_intermixed_lst.append(prot_2_feat_lst[i])
            wrapper_feat_lst.append(np.concatenate((row.to_numpy(), np.asarray(feat_intermixed_lst)), axis=0))
...
if __name__ == '__main__':
    ...
    for dataset_type in dataset_type_lst:
        data_prep_feat_intermixed(root_path, dataset_type)
