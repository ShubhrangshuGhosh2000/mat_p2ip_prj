...
# prepare the dataset by the feature multiplication for the given dataset type
def data_prep_feat_multiplied(root_path='./', dataset_type='Random50'):
    print("\n ############ inside the data_prep_feat_multiplied() method ############")
    input_path = os.path.join(root_path, 'dataset/orig_data/Human2021/BioGRID2021', dataset_type)
    output_path = os.path.join(root_path, 'dataset/preproc_data/human_2021', dataset_type, 'feat_multiplied')
    ...
    # iterate over the tsv files and perform the feature multiplication
    for indiv_tsv_file_nm in tsv_file_names_lst:
        ...
        for ind, row in indiv_tsv_df.iterrows():
            ...
            # elementwise multiply two feature arrays
            feat_product_arr = np.multiply(prot_1_feat_arr, prot_2_feat_arr)
            wrapper_feat_lst.append(np.concatenate((row.to_numpy(), feat_product_arr), axis=0))
...
if __name__ == '__main__':
    ...
    for dataset_type in dataset_type_lst:
        data_prep_feat_multiplied(root_path, dataset_type)
